...
def dqn_grid_search():
    ...
    for params in grid:
        agent = Agent(env.observation_space.shape[0], env.action_space.n, **params)
        curr_step = 0
        episode_rewards = 0
        all_episode_rewards = []
        all_20_epi_rewards = []
        ...
        print(f"params: {params} || Max reward {max(all_episode_rewards)} || Max average reward {max(all_20_epi_rewards)}")
